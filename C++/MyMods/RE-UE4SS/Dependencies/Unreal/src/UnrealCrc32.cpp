...
uint32_t FCrc::CRCTable_DEPRECATED[256] =
        {
...
uint32_t FCrc::CRCTablesSB8_DEPRECATED[8][256] =
        {
                {
...
uint32_t FCrc::memcrc32(const void* data, int32_t length, uint32_t CRC)
{
    ...
    const uint8_t* __restrict Data = (uint8_t*) data;

    // First we need to align to 32-bits
    int32_t InitBytes = static_cast<int32_t>(Align(Data, 4) - Data);
...
uint32_t FCrc::memcrc_deprecated(const void* data, int32_t data_length, uint32_t CRC)
{
    // Based on the Slicing-by-8 implementation found here:
    // http://slicing-by-8.sourceforge.net/
    ...
    if (data_length > InitBytes)
    {
        data_length -= InitBytes;
        ...
        auto Data4 = (const uint32_t*) Data;
        for (uint32_t Repeat = data_length / 8; Repeat; --Repeat)
        {
            uint32_t V1 = *Data4++ ^ CRC;
            uint32_t V2 = *Data4++;
            CRC =
